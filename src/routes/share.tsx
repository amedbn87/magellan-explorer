...
export const Route = createFileRoute("/share")({
  head: () => ({ meta: [{ title: "Share Location — Magellan" }, { name: "description", content: "Share a versioned Magellan location payload." }] }),
  component: SharePage,
});
...
function SharePage() {
  ...
  const payload: LocationPayloadV1 | null = useMemo(() => {
    if (sourceId === "live") {
      if (!captured?.latitude === undefined || captured?.latitude === undefined || captured.longitude === undefined) return null;
      return {
        t: "MGLN", v: MAGELLAN_PAYLOAD_VERSION,
        lat: captured.latitude, lon: captured.longitude,
        alt: captured.altitudeM, acc: captured.accuracyM,
        ts: Math.floor(captured.timestamp / 1000),
        name: name.trim() || undefined, note: note.trim() || undefined,
        src: captured.isNative ? "live" : "demo",
      };
    }
  ...
  function generateQr() {
    if (!payload) { toast.error("No valid location is available"); return; }
  ...
  function handleSend() {
    if (!payload) return;
    if (transport === "qr" && !qrValue) { toast.error("Generate the QR code first"); return; }
    const result = send(transport, payload);
    if (result.ok) {
      addHistory({ kind: "shared", transport, label: payload.name ?? "Shared location", latitude: payload.lat, longitude: payload.lon, accuracyM: payload.acc });
  ...
  return (
    <div className="space-y-5">
      <PageHeader title={t("nav_share")} description="Prepare a point-in-time MGLN v1 payload without doing heavy work during page load." />
      <div className="rounded-lg border border-border bg-card p-4 text-sm text-muted-foreground">Share Location is intentionally idle until you request QR generation. This prevents QR encoding from competing with Android WebView navigation and touch handling.</div>
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="space-y-3 rounded-lg border border-border bg-card p-4">
          ...
          {qrValue ? <><QrCanvas value={qrValue} /><div className="rounded-md border border-border bg-muted/40 p-3"><div className="label-eyebrow">Encoded payload</div><pre className="numeric mt-1 overflow-x-auto whitespace-pre-wrap break-all text-[11px]">{qrValue}</pre></div><Button variant="outline" className="w-full" onClick={() => { if (!navigator.clipboard?.writeText) return void toast.error("Clipboard unavailable"); void navigator.clipboard.writeText(qrValue).then(() => toast.success("Payload copied")).catch(() => toast.error("Could not copy payload")); }}>Copy payload</Button></> : <div className="grid min-h-64 place-items-center rounded-lg border border-dashed border-border text-center text-sm text-muted-foreground p-6">The QR is not generated yet. This screen remains interactive until you explicitly request it.</div>}
        </div>
      </div>
    </div>
  );
