...
function Index() {
  ...
  for (const sat of s.satellites) {
    ...
    e.v += 1; if (sat.usedInFix) e.u += 1; byConstellation.set(sat.constellation, e);
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-[220px_minmax(0,1fr)]">
          <div className="grid place-items-center rounded-xl border border-border bg-card p-5">
            <div className="relative grid h-44 w-44 place-items-center rounded-full border-2 border-border">
              {["N", "E", "S", "W"].map((label, i) => <span key={label} className="absolute text-sm font-bold" style={{ transform: `rotate(${i * 90}deg) translateY(-5rem) rotate(${-i * 90}deg)` }}>{label}</span>)}
              <Compass className="h-28 w-28 text-primary transition-transform duration-200" style={{ transform: `rotate(${heading ?? 0}deg)` }} aria-label={heading !== undefined ? `Heading ${heading.toFixed(0)} degrees` : "Heading unavailable"} />
            </div>
            <div className="numeric mt-3 text-2xl font-semibold">{heading !== undefined ? `${heading.toFixed(0)}° ${cardinal(heading)}` : "—"}</div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Stat label="Heading" value={heading?.toFixed(0)} unit="°" hint={headingSource === "unavailable" ? "Sensor unavailable" : headingSource === "compass" ? "Device orientation" : "GNSS course"} />
      ...
      <Section title="Nearby locations" aside={<Link className="text-xs text-primary" to="/waypoints">Manage all</Link>}>
        <div className="space-y-2">
          {nearby.map(({ waypoint, distance }) => {
            ...
            return <button key={waypoint.id} className="flex w-full items-center gap-3 rounded-lg border border-border bg-card p-3 text-left hover:bg-secondary" onClick={() => { window.location.assign(`/navigate?waypoint=${encodeURIComponent(waypoint.id)}`); }}><MapPin className="h-4 w-4 shrink-0 text-primary" /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{waypoint.name}</span><span className="block truncate text-xs text-muted-foreground">{group?.name ?? "Ungrouped"}</span></span><span className="numeric text-sm font-medium">{distance !== undefined ? formatDistance(distance) : "Distance unavailable"}</span></button>;
          })}
      ...
      <Section title="Location overview">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Stat label="Saved locations" value={waypoints.length} /><Stat label="Groups" value={groups.length} /><Stat label={t("accuracy")} value={s.accuracyM?.toFixed(1)} unit="m" /><Stat label={t("altitude")} value={s.altitudeM?.toFixed(0)} unit="m" /></div>
        <div className="mt-3 rounded-lg border border-border bg-card p-4"><div className="label-eyebrow">Current coordinates (WGS84)</div><p className="numeric mt-1 text-xl font-semibold">{s.latitude !== undefined && s.longitude !== undefined ? formatCoord(s.latitude, s.longitude) : "Unavailable"}</p><p className="mt-1 text-xs text-muted-foreground">Updated {new Date(s.timestamp).toLocaleString()} · Source: {s.source}</p></div>
      </Section>
