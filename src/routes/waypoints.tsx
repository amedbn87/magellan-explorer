...
import { OpenStreetMapPicker } from "@/components/magellan/OpenStreetMapPicker";

export const Route = createFileRoute("/waypoints")({ head: () => ({ meta: [{ title: "Locations & Groups — Magellan" }] }), component: WaypointsPage });
const empty = { name: "", lat: "", lon: "", note: "", groupId: "", source: "manual" as "manual" | "live" | "map" | "received" };
...
function WaypointsPage() {
  ...
  function fillCurrent() {
    ...
    setForm((current) => ({ ...current, lat: String(snapshot.latitude), lon: String(snapshot.longitude), source: snapshot.isNative ? "live" : "manual" }));
    setAddMode("current");
  }
  ...
  function beginEdit(w: Waypoint) { setEditing(w.id); setAddMode(null); setForm({ name: w.name, lat: String(w.latitude), lon: String(w.longitude), note: w.note ?? "", groupId: w.groupId ?? "", source: w.source === "received" ? "received" : w.source === "map" ? "map" : w.source === "live" ? "live" : "manual" }); }
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-lg border border-border bg-card p-4"><div className="mb-3"><h2 className="font-semibold">Add location</h2><p className="text-xs text-muted-foreground">Choose the real current GNSS position or select a point on OpenStreetMap.</p></div><div className="grid gap-2 sm:grid-cols-2"><Button variant="outline" onClick={fillCurrent}>Use current location</Button><Button variant="outline" onClick={() => setAddMode("map")}><Map className="mr-2 h-4 w-4" />Select on OpenStreetMap</Button></div>{addMode === "map" ? <div className="mt-4"><OpenStreetMapPicker initial={{ latitude: snapshot.latitude ?? 24.7136, longitude: snapshot.longitude ?? 46.6753 }} onConfirm={saveMapLocation} /><Button className="mt-2" type="button" variant="ghost" onClick={cancelEditor}>Cancel map selection</Button></div> : null}</section>
