...
function fishingActivity(input: {
  ...
  if (input.pressure !== undefined)
    factors.push(input.pressure >= 1008 && input.pressure <= 1025 ? 2 : input.pressure >= 995 ? 1 : 0);
  ...
  const nearSolunar = [sunrise, sunset].some(
    (t) => Number.isFinite(t) && Math.abs(now - t) <= 2 * 3600000,
  );
  if (nearMajorMoon) factors.push(2);
  else factors.push(1);
...
export async function fetchMarineConditions(
  ...
  const marineParams = new URLSearchParams({
    ...
    longitude: longitude.toFixed(5),
    current:
      "wave_height,wave_period,wave_direction,sea_surface_temperature,ocean_current_velocity,ocean_current_direction",
    timezone: "auto",
    forecast_days: "1",
  });
