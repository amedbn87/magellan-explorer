  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <div className="space-y-4 rounded-lg border border-border bg-card p-4">
        ...
        <div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={generateQr} disabled={!payload}>{qrValue ? "Regenerate QR" : "Generate QR"}</Button><Button onClick={handleSend} disabled={!payload || (transport === "qr" && !qrValue)}>Send via {active?.name ?? "transport"}</Button></div>
      </div>
