...
export function MagellanProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<MagellanState>(() => {
    ...
    return {
      ...
      addWaypoint: (w) => { const wp: Waypoint = { ...w, id: uid(), createdAt: Date.now() }; persistWaypoints([wp, ...waypoints]); return wp; },
      ...
      clearHistory: () => { setHistory([]); saveHistory([]); },
      lang, setLang, theme, setTheme, t: (k) => translate(lang, k),
    };
